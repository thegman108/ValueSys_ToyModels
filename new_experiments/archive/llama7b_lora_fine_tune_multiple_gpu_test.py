...
#now passing in the rank, and world_size
#def train(rank,world_size,epochs,token, log_interval=10,training_type=None):
def train(epochs,token, log_interval=10,training_type=None):
    ...
    #Now including items for distributed training
    ...
    print("WandB logging initialized for main process.")
   
        
    #removing in order to have multiple GPUs
    ...
    #train_loader = DataLoader(data, batch_size=train_params.per_device_train_batch_size, shuffle=False, sampler=sampler)
    ##############TRAIN###############
    ...
    #eval_loader = DataLoader(data_v, batch_size=train_params.per_device_train_batch_size, sampler=sampler)
    ##############VALIDATION###############
    ...
    #pretty sure i dont need this now that I am just writing things to one gpu.
...
def main():
    ...
    mp.spawn(train, args=(world_size, epoch_count, token, log_interval, training_type), nprocs=world_size, join=True)
    train(epochs=epoch_count,token=token,training_type=training_type)
